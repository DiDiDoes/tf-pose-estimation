...
class EfficientnetNetwork1(network_base.BaseNetwork):
    ...
    def setup(self):
        ...
        self.feed('image').base(name='efficientnet-b1')

        # for n, l in enumerate(self.layers):
        #     print(l)

        self.feed('efficientnet-b1/block_4').max_pool(2, 2, 2, 2, name='efficientnet-b1/block_4_pool')
        self.feed('efficientnet-b1/block_15').upsample(factor='efficientnet-b1/block_7', name='efficientnet-b1/block_15_upsample')
        (self.feed(
            'efficientnet-b1/block_4_pool',
            'efficientnet-b1/block_7',
            'efficientnet-b1/block_15_upsample',
    ...
    def restorable_variables(self, only_backbone=True):
        vs = {v.op.name: v for v in tf.global_variables() if
              ('efficientnet-b1' in v.op.name or (only_backbone is False and 'Openpose' in v.op.name)) and
...
if __name__ == '__main__':
    ...
    with tf.variable_scope(tf.get_variable_scope(), reuse=False):
        network1 = EfficientnetNetwork1({'image': input1})

    num_params = np.sum([np.prod(v.shape) for v in tf.trainable_variables()])
    print(num_params)

    with tf.variable_scope(tf.get_variable_scope(), reuse=True):
        network2 = EfficientnetNetwork1({'image': input2})
