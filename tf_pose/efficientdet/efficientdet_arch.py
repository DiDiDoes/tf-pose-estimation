...
def efficientdet(features, model_name=None, config=None, **kwargs):
  ...
  print('backbone params/flops = {:.6f}M, {:.9f}B'.format(
      *utils.num_params_flops()))

  # build feature network.
  fpn_feats = build_feature_network(features, config)
  print('backbone+fpn params/flops = {:.6f}M, {:.9f}B'.format(
      *utils.num_params_flops()))

  endpoints = {}
  endpoints['P3'] = fpn_feats[3]
  endpoints['P4'] = fpn_feats[4]
  endpoints['P5'] = fpn_feats[5]
  endpoints['P6'] = fpn_feats[6]
  endpoints['P7'] = fpn_feats[7]

  return fpn_feats, endpoints
